    ...
    <a-breadcrumb :routes="routes">
      <template #itemRender="{ route, routes: routesMatched, paths }">
        ...
        <span v-if="!hasRedirect(routesMatched, route)">
          ...
          }}<span v-if="getTitleSuffix(route)" style="margin-left: 10px">{{
            getTitleSuffix(route)
          }}</span>
        </span>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      watchEffect(async () => {
        ...
        if (
          breadcrumbList.some((item) => item.path.indexOf('/pages/') > -1 || item.meta?.isCommon)
        ) {
      ...
      const getTitleSuffix = computed(() => {
        return function (route: any) {
          if (route.path == currentRoute.value.path) {
            const titleSuffix = currentRoute.value.query.titleSuffix as string;
