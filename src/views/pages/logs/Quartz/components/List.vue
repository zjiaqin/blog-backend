      ...
      <template #tableTitle>
        <div class="flex w-full">
          <AButton
            ...
            danger
            class="mr-2"
            :loading="btnLoading.loading && btnLoading.btnId === -2"
            @click="toDelete()"
            >{{ ModalStatuEnum.BATCHEDELE }}</AButton
          >
          <AButton type="primary" danger class="mr-2" @click="toClean()">{{
            ModalStatuEnum.CLEAN
          }}</AButton>
        </div>
      </template>
...
<script setup lang="ts">
  ...
  async function toDelete(id: _listResp[number]['id'] = -1) {
    ...
    try {
      await deleteJobLogsUsingDelete({ requestBody: ids });
  ...
  const toClean = async () => {
    btnLoading.btnId = -2;
    btnLoading.loading = true;
    await cleanJobLogsUsingDelete(undefined);
    btnLoading.loading = false;
    reload();
  };
  ...
  onMounted(() => {
    getForm().setFieldsValue({ jobId: route.query.jobId });
  });
</script>
