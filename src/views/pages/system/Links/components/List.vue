  ...
  <div class="link-list-table">
    <BasicTable @register="registerTable" title=" ">
      <template #tableTitle>
        <div class="flex w-full">
          <AButton
            ...
            class="mr-2"
            @click="toEdit(ModalStatuEnum.ADD)"
      ...
      <template #bodyCell="{ column, record }: { column: any, record: _listResp[0] }">
        <template v-if="column.key === 'linkAvatar'">
          <Image
            :width="100"
        ...
        <template v-if="column.key === 'operate'">
          <AButton
            ...
            type="primary"
            @click="toEdit(ModalStatuEnum.EDIT, record)"
...
<script setup lang="ts">
  ...
  import { listFriendLinkDtoUsingGet, deleteFriendLinkUsingDelete } from '/@/api/apis';
  ...
  async function toDelete(id: _listResp[number]['id'] = -1) {
    ...
    try {
      await deleteFriendLinkUsingDelete({ requestBody: ids });
  ...
  const toEdit = (mode, record: _listResp[number] = {}) => {
    setModalProps({ title: mode + '友链' });
